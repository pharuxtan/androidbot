module.exports = (globalVariables) => {
  ...
  async function command(message){
    try {
      ...
      if(srch !== ""){
        let names = Object.values(device).map(n => n.join("/"));
        let keys = Object.keys(device);
        let result = FuzzySearch(names, srch).map(n => {
          let search = FuzzySearch(device[keys[n]], srch);
          if(search.length === 0) return null;
          return `\`${keys[n]}\`: ${search.map(num => device[keys[n]][num]).join(" | ")}`;
        }).filter(n => n !== null);
        let s;
        if(result.length <= 1){
          s = "";
        ...
        if(result[0] === undefined){
          message.channel.send(`No codename was found for \`${srch}\``);
        } else {
          let page = 1;
  ...
  command.help = {
    "category": "global",
    "description": "Search a codename with a phone name",
    "example": "{prefix}cdn Xiaomi Redmi Note 7 Pro"
  }
