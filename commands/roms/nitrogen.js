module.exports = (globalVariables) => {
  ...
  async function command(message){
    ...
    let msg = await message.channel.send(`Searching official NitrogenOS for \`${devicename(codename)}\`...`);
    try {
      let codenames = Object.keys(JSON.parse((await fetch("https://sourceforge.net/projects/nitrogen-project/files/").then(res => res.text())).split("net.sf.files = ")[1].split(";")[0]));
      if(!codenames.includes(codename)){
        if(codenames.includes(codename.toUpperCase())) codename = codename.toUpperCase();
      }
      if(codenames.includes(codename)){
        let file = JSON.parse((await fetch(`https://sourceforge.net/projects/nitrogen-project/files/${codename}/`).then(res => res.text())).split("net.sf.files = ")[1].split(";")[0]);
        if(Object.keys(file).includes("10")){
          file = JSON.parse((await fetch(`https://sourceforge.net/projects/nitrogen-project/files/${codename}/10`).then(res => res.text())).split("net.sf.files = ")[1].split(";")[0]);
        }
        let files = Object.keys(file).filter(f => f.endsWith(".zip")).sort((a,b) => (parseFloat(b.split("-")[4])-parseFloat(a.split("-")[4])));
        if(files.length == 0) return msg.edit(`NitrogenOS wasn't find for \`${devicename(codename)}\``);
        let official = files[0];
        if(official == undefined) return msg.edit(`NitrogenOS wasn't find for \`${devicename(codename)}\``);
        let head = await fetch((await handleSF(file[official].download_url))[0], {method: "HEAD"});
        let download = file[official].download_url.replace("/download", `?r=&ts=${Math.floor(Date.now() / 1000)}&use_mirror=autoselect`).replace("https://sourceforge.net/projects/nitrogen-project/files/","https://downloads.sourceforge.net/project/nitrogen-project/");
        let embed = new Discord.MessageEmbed()
          .setTitle(`NitrogenOS | ${devicename(codename).split(" | ")[0]}`)
        ...
        msg.edit(`NitrogenOS wasn't find for \`${devicename(codename)}\``);
      }
    } catch(e){
